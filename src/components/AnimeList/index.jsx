  ...
  return (
    <div className="grid md:grid-cols-4 sm:grid-cols-3 grid-cols-2 gap-4 m-5 ">
      {api_data.data.map((anime) => {
        return (
          <div key={anime.mal_id} className="text-color-primary hover:text-color-accent transition-all">
            <Link href={`/${anime.mal_id}`} clasName="cursor-pointer">
              <Image src={anime.images.webp.image_url} alt={anime.title} width={350} height={350} className="w-full max-h-96 objec-cover" />
